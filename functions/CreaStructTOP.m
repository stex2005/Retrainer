...
for i=1:length(IndexexerciseLine)
    ...
    if (strcmp(exercise{IndexexerciseLine(i)},Name) == 0 && strcmp(exercise{IndexexerciseLine(i)},'get_ExercisesDone') == 0)
    ...
    if ((strcmp(exercise{IndexexerciseLine(i)},'get_ExercisesDone') == 1) || strcmp(startState{IndexexerciseLine(i)}(end-4:end),'_Stop'))
        Name='';
    end
end
...
for i=1:length(IndexexerciseLine)
    if ((strcmp(exercise{IndexexerciseLine(i)},Name) == 0 && strcmp(exercise{IndexexerciseLine(i)},'get_ExercisesDone') == 0) || strcmp(startState{IndexexerciseLine(i)}(end-4:end),'_Stop'))
        ex_index=ex_index+1;
    ...
    for r=1:max(size(S1exerciseOK{j}.Task))
        ...
        for n=1:max(size(S1exerciseOK{j}.Task{r}.rep))
            ...
            if (ID)
                ...
                S1exerciseOK{j}.Task{r}.rep{n}.time_inv=S1exerciseOK{j}.Task{r}.rep{n}.AbsTimeMS(ID);
...
for r=1:4
    ...
    for n=1:max(size(S1exerciseOK{i_ex}.Task{r}.rep))
        if(isempty(S1exerciseOK{i_ex}.Task{r}.rep{n}.timeI)==0)
            ...
            hold on
            plot(S1exerciseOK{i_ex}.Task{r}.rep{n}.timeI - S1exerciseOK{i_ex}.Task{r}.rep{n}.timeI(1), S1exerciseOK{i_ex}.Task{r}.rep{n}.PW1,'b')
            plot(S1exerciseOK{i_ex}.Task{r}.rep{n}.timeI- S1exerciseOK{i_ex}.Task{r}.rep{n}.timeI(1),S1exerciseOK{i_ex}.Task{r}.rep{n}.PW2,'r')
        end
    end
    legend('M1','M2')
    xlabel('time [ms]')
