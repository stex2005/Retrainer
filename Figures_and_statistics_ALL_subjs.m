...
%         temp_smoothness=[];
        for t=1:NR_task
            NR_rep=max(size(S1exerciseOK_2{exe_to_consider}.Task{t}.rep));
...
%             ROM_SR(t)=S1exerciseOK_2{exe_to_consider}.Task{t}.ROM_SR_mean;
            for x=1:NR_rep
                if (isfield(S1exerciseOK_2{exe_to_consider}.Task{t}.rep{x},'inv_1'))
                INV_1(x)=S1exerciseOK_2{exe_to_consider}.Task{t}.rep{x}.inv_1;
                end
                if (isfield(S1exerciseOK_2{exe_to_consider}.Task{t}.rep{x},'inv_2'))
                INV_2(x)=S1exerciseOK_2{exe_to_consider}.Task{t}.rep{x}.inv_2;
                end
        end
...
%         CP_std_mat(n,d)=CP_std_task(d);
        inv_1_mean_task(d)=nanmean(INV_1);
        inv_2_mean_task(d)=nanmean(INV_2);
        inv_1_mean_mat(n,d)=inv_1_mean_task(d);
        inv_2_mean_mat(n,d)=inv_2_mean_task(d);
        inv_1_std_task(d)=nanstd(INV_1);
        inv_2_std_task(d)=nanstd(INV_2);
        inv_1_std_mat(n,d)=inv_1_std_task(d);
        inv_2_std_mat(n,d)=inv_2_std_task(d);
...
%     ylabel('shoulder rotation [°]') 
    ...
    switch n
        case 1
            plot([1 2],inv_1_mean_task,'--*','Color','k','MarkerSize',12)
            hold on
%             errorbar([1 2],inv_1_mean_task,inv_1_std_task,'k')
        case 2
            plot([1.1 2.1],inv_1_mean_task,'--*','Color','r','MarkerSize',12)
            hold on
%             errorbar([1.1 2.1],inv_1_mean_task,inv_1_std_task,'r')
        case 3
            plot([1.2 2.2],inv_1_mean_task,'--*','Color','b','MarkerSize',12)
            hold on
%             errorbar([1.2 2.2],inv_1_mean_task,inv_1_std_task,'b')
        case 4
            plot([1.3 2.3],inv_1_mean_task,'--*','Color','m','MarkerSize',12)
            hold on
%             errorbar([1.3 2.3],inv_1_mean_task,inv_1_std_task,'m')
        case 5
            plot([1.4 2.4],inv_1_mean_task,'--*','Color','g','MarkerSize',12)
            hold on
%             errorbar([1.4 2.4],inv_1_mean_task,inv_1_std_task,'g')    
        case 6
            plot([0.9 1.9],inv_1_mean_task,'--*','Color',[0 0.6 0],'MarkerSize',12)
            hold on
%             errorbar([0.9 1.9],inv_1_mean_task,CP_inv_1_std_task,'Color',[0 0.6 0])
    end
    title('Involvement 1 [0-1]','FontSize',14,'FontWeight','b')
    set(gca,'XTick',[1 2.05])
    set(gca,'XTickLabel',['Day 1';'Day N'])
    set(gca,'Fontsize',14)
    xlim([0.5 3])
    ...
    switch n
        case 1
            plot([1 2],inv_2_mean_task,'--*','Color','k','MarkerSize',12)
            hold on
%             errorbar([1 2],inv_2_mean_task,inv_2_std_task,'k')
        case 2
            plot([1.1 2.1],inv_2_mean_task,'--*','Color','r','MarkerSize',12)
            hold on
%             errorbar([1.1 2.1],inv_2_mean_task,inv_2_std_task,'r')
        case 3
            plot([1.2 2.2],inv_2_mean_task,'--*','Color','b','MarkerSize',12)
            hold on
%             errorbar([1.2 2.2],inv_2_mean_task,inv_2_std_task,'b')
        case 4
            plot([1.3 2.3],inv_2_mean_task,'--*','Color','m','MarkerSize',12)
            hold on
%             errorbar([1.3 2.3],inv_2_mean_task,inv_2_std_task,'m')
        case 5
            plot([1.4 2.4],inv_2_mean_task,'--*','Color','g','MarkerSize',12)
            hold on
%             errorbar([1.4 2.4],inv_2_mean_task,inv_2_std_task,'g')    
        case 6
            plot([0.9 1.9],inv_2_mean_task,'--*','Color',[0 0.6 0],'MarkerSize',12)
            hold on
%             errorbar([0.9 1.9],inv_2_mean_task,CP_inv_2_std_task,'Color',[0 0.6 0])
    end
    title('Involvement 2 [0-1]','FontSize',14,'FontWeight','b')
    set(gca,'XTick',[1 2.05])
    set(gca,'XTickLabel',['Day 1';'Day N'])
    set(gca,'Fontsize',14)
    xlim([0.5 3])
...
p_inv_1_mean=signrank(inv_1_mean_mat(:,1),inv_1_mean_mat(:,2));
p_inv_1_std=signrank(inv_1_std_mat(:,1),inv_1_std_mat(:,2));
p_inv_2_mean=signrank(inv_2_mean_mat(:,1),inv_2_mean_mat(:,2));
p_inv_2_std=signrank(inv_2_std_mat(:,1),inv_2_std_mat(:,2));
